...
void Merekle_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Merekle_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 7084;}

uint get_number_of_components() {return 10;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 5289;}
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[17]); // line circom 16
while(Fr_isTrue(&expaux[0])){
{
...
ctx->componentMemory[coffset].templateName = "Merekle";
ctx->componentMemory[coffset].signalStart = soffset;
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[17]); // line circom 12
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 0;
{
...
PFrElement aux_dest = &lvar[1];
// load src
Fr_add(&expaux[0],&lvar[1],&circuitConstants[3]); // line circom 12
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[17]); // line circom 12
...
Merekle_2_create(1,0,ctx,"main",0);
Merekle_2_run(0,ctx);
}
